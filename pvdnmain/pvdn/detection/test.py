...
def test(data_path, conf_thresh, output_dir, model_path, save_coco, plot_scenes,
    ...
    print("öutput directory , device " , output_dir , device) 
    # set up output path
    ...
    try:
        # model.load_state_dict(torch.load(os.path.abspath(model_path))["model"])
         model.load_state_dict(torch.load(os.path.abspath(model_path))["model"],map_location=torch.device(device), weights_only=True)
    except KeyError:
        # model.load_state_dict(torch.load(os.path.abspath(model_path)))
        model.load_state_dict(torch.load(os.path.abspath(model_path)),map_location=torch.device(device), weights_only=True)
